...
(72 such people, 150 notes). So a name here is a person, and `[[wikilinks]]` to
...
**The filename is the identity, not the display name.** OpenAlex spells the same
person differently across papers (`Ramon Diaz‐Arrastia` with a non-ASCII hyphen on
six papers, `Ramon Diaz Arrastia` on a seventh), and `slugify` folds those
spellings onto one filename — so papers link as `[[Ramon Diaz Arrastia|Ramon
Diaz‐Arrastia]]`, target first, that paper's own spelling as the alias. When
counting or grouping people, key on the target. Keying on the alias splits one
person into several, each holding a share of their papers.
